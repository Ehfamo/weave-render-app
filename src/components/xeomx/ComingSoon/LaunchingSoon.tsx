...
export function LaunchingSoon({ section }: { section: ExploreSection }) {
  ...
  return (
    <div className="relative min-h-svh overflow-hidden bg-background">
      ...
      />
      <div
        className="pointer-events-none absolute -left-40 top-20 h-80 w-80 rounded-full opacity-25 blur-3xl"
      ...
      <div className="relative mx-auto flex max-w-3xl flex-col gap-10 px-4 py-10 sm:px-8 sm:py-16">
        ...
        <div>
          <span
            className="grid h-20 w-20 place-items-center rounded-3xl ring-1 ring-amber-300/30"
            style={{ background: "var(--gradient-gold)" }}
          >
        ...
        {section.features && section.features.length > 0 && (
          <ul className="grid gap-3 sm:grid-cols-2">
            {section.features.map((f) => (
              <li
                key={f}
                className="flex items-start gap-3 rounded-2xl border border-white/10 bg-surface/40 p-4 backdrop-blur"
              >
                <span
        ...
        <div className="rounded-3xl border border-amber-300/20 bg-surface/40 p-6 backdrop-blur">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {m.launching_waitlist_desc({ name: section.name })}
          </p>
          <div className="mt-4">
            <NotifyForm label={m.notify_waitlist_button()} sectionSlug={section.slug} />
          </div>
        </div>
      </div>
    </div>
