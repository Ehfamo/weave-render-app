...
function useContextState() {
  ...
  const context = useMemo(
    () => (bound.actorId === (user?.id ?? null) ? bound.context : createEmptyProjectContext()),
    [bound, user?.id],
  );
  const rememberHandoff = useCallback(
    (location: ProjectLocation) => {
      setBound((current) => ({
        ...
        context: recordProjectHandoff(
          current.actorId === (user?.id ?? null) ? current.context : createEmptyProjectContext(),
          location,
        ),
      }));
    },
  ...
  const bindProject = useCallback(
    (projectId: string, actorId: string) => {
      ...
      setBound((current) =>
        current.actorId === actorId && current.context.projectId === next.projectId
          ? current
          : { actorId: user?.id ?? null, context: next },
      );
    },
    [user?.id],
  );
  const clearProject = useCallback((projectId: string) => {
    setBound((current) =>
      current.context.projectId === projectId
        ? { actorId: null, context: createEmptyProjectContext() }
        : current,
    );
  }, []);
  return useMemo(
    () => ({ context, rememberHandoff, bindProject, clearProject }),
    [context, rememberHandoff, bindProject, clearProject],
  );
}
