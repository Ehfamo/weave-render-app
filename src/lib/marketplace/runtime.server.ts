...
export async function marketplaceForRequest(authorization: string | null) {
  ...
  const client=createClient(url,key,{ auth:{ persistSession:false,autoRefreshToken:false }, ...(authorization ? { global:{ headers:{ Authorization:authorization } } } : {}) });
  ...
  return new MarketplaceService(new SupabaseMarketplaceStore(userId,client,supabaseAdmin as unknown as SupabaseClient),sandboxRuntime(actor=>({ ...ephemeralServices(actor,actor),gateway:createModelGatewayRuntime().gateway })));
}
