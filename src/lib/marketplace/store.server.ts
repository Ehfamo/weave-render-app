...
export class SupabaseMarketplaceStore implements MarketplaceStore {
  ...
  async get(id: string) {
    const visible = await database(
      this.client.from("marketplace_listings").select("id,state").eq("id", id).maybeSingle(),
    );
  ...
  async publish(entry: CatalogEntry) {
    return (await database(
      this.admin.rpc("xeomx_marketplace_publish", { p_actor: this.userId, p_entry: entry }),
    )) as CatalogEntry;
  }
  ...
  async projectContext(id: string) {
    ...
    return (
      await projectsForClient(this.userId!, this.client).brain.buildContext(id, {
        maxCharacters: 4000,
      })
    ).text;
  }
  async grant(packageId: string, projectId: string) {
    const row = await database(
      this.client
        .from("marketplace_permission_grants")
        .select("record")
  ...
  async saveGrant(entry: CatalogEntry, projectId: string) {
    await database(
      this.client.rpc("xeomx_marketplace_grant", {
        p_version: entry.id,
        p_project: projectId,
        p_digest: entry.manifest.integrity.digest,
        p_permissions: entry.manifest.permissions,
      }),
    );
  }
  async claimTrial(record: TrialRecord) {
    return (await database(
      this.admin.rpc("xeomx_marketplace_trial", {
        p_actor: this.userId,
        p_action: "claim",
        p_record: record,
      }),
    )) as { claimed: boolean; record: TrialRecord };
  }
  ...
  async review(id: string, dimensions: Record<string, number>, text: string) {
    await database(
      this.client.rpc("xeomx_marketplace_review", {
        p_version: id,
  ...
  async reviews(id: string) {
    const rows = await database(
      this.client
        ...
        .select("dimensions,body")
        .eq("version_id", id)
        .eq("status", "active")
        .limit(50),
    );
    return (rows ?? []).map((x) => ({
      dimensions: x.dimensions as Record<string, number>,
      text: x.body as string,
    }));
  }
  async continuation(id: string, kind: "job" | "conversation") {
    ...
    if (kind === "job") {
      const row = await database(
        this.client
          ...
          .select("id,project_id,runtime_data,state,failure_code")
          .eq("id", id)
          .eq("requested_by", this.userId)
          .single(),
      );
      if (!row) throw Error("TASK_UNAVAILABLE");
      await this.authorizeProject(row.project_id);
      if (!row.runtime_data?.request?.task?.goal || !["failed", "unavailable"].includes(row.state))
        throw Error("TASK_NOT_WAITING_FOR_CAPABILITY");
      return {
        projectId: row.project_id as string,
        referenceId: id,
        kind,
        goal: String(row.runtime_data.request.task.goal).slice(0, 500),
      };
    }
    const row = await database(
      this.client
        .from("conversations")
        .select("id,project_id,core_execution")
        .eq("id", id)
    ...
    if (!row) throw Error("TASK_UNAVAILABLE");
    await this.authorizeProject(row.project_id);
    if (!["FAILED", "NOT_CONFIGURED"].includes(row.core_execution?.state))
      throw Error("TASK_NOT_WAITING_FOR_CAPABILITY");
    ...
    return {
      ...
      referenceId: id,
      kind,
      goal: String(messages[0].content).slice(0, 500),
    };
  }
}
