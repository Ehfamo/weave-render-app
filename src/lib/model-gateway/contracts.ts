...
export interface ProviderMetadata { id: string; displayName: string }
...
export type ModelErrorCode = "AUTH_ERROR" | "RATE_LIMIT" | "PROVIDER_UNAVAILABLE" |
  "INVALID_REQUEST" | "TIMEOUT" | "CONTENT_REJECTED" | "UNKNOWN_PROVIDER_ERROR";
export interface ModelError { code: ModelErrorCode; message: string; retryable: boolean }
export type Availability = "AVAILABLE" | "UNAVAILABLE" | "DEGRADED";
export interface ProviderHealth { availability: Availability; checkedAt: string }
export type AdapterResult =
  | { ok: true; output: ModelOutput; usage?: ModelUsage }
...
export interface ProviderAdapter {
  ...
  execute(request: Readonly<ModelRequest>, model: Readonly<ModelIdentity>, signal?: AbortSignal): Promise<AdapterResult>;
}
