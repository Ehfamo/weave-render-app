...
export interface CostBudget {
  maxRunMinor?: MoneyMinor;
  warningAtMinor?: MoneyMinor;
...
export interface ProviderCostMetadata {
  model: ModelIdentity;
  inputPerMillionMinor?: MoneyMinor;
  outputPerMillionMinor?: MoneyMinor;
  currency: string;
  effectiveAt: string;
}
...
export interface CostRouteDecision {
  status: "selected" | "budget_exceeded" | "unavailable";
  model?: ModelIdentity;
...
export type QualityDimension =
  "structure" | "provenance" | "continuity" | "brand" | "code" | "security" | "cost" | "contract";
...
export interface QualityGate {
  minimumScore: number;
  required: readonly QualityDimension[];
...
export interface ReleaseGateResult {
  status: GateStatus;
  dimensions: Readonly<Record<string, GateStatus>>;
