...
export const projectsHomeFn = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(({ context }) => safely(async () => (await service(context)).home()));
export const projectWorkspaceFn = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(({ context, data }) =>
    safely(async () => (await service(context)).open(data.projectId, data.conversationId)),
  );
export const createDurableProjectFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((value: unknown) => {
    const v = object(value);
    return { name: v.name as string, goal: v.goal as string | undefined };
  })
  .handler(({ context, data }) => safely(async () => (await service(context)).create(data)));
export const editDurableProjectFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((value: unknown) => {
    const v = object(value);
    return {
  ...
  .handler(({ context, data }) =>
    safely(async () => (await service(context)).edit(data.projectId, data)),
  );
export const memoryControlFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((value: unknown) => value)
  .handler(({ context, data }) =>
    safely(async (): Promise<Awaited<ReturnType<ProjectsService["memoryControl"]>>> =>
