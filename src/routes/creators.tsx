...
export const Route = createFileRoute("/creators")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Founder & elite prompt engineers earning from copies, saves, and remixes.",
      },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      >
        ...
        >
          {m.creators_title_1()}{" "}
          <span className="text-gradient-magenta italic">{m.creators_title_2()}</span>
        </h1>
        <p
          className="max-w-xl"
          style={{
        ...
        {isLoading ? (
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="h-80 animate-pulse rounded-3xl border border-border/60 bg-surface/40"
              />
            ))}
          </div>
        ) : error ? (
          ...
          <div className="mt-10 rounded-3xl border border-dashed border-border/60 p-10 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Publish your first prompt to appear here.
            </p>
          </div>
        ) : (
          <motion.div
                ...
                >
                  <CreatorCard
                    ...
                    onFollow={() => {
                      if (!authorId) return;
                      if (!uid) {
                        navigate({ to: "/auth", search: { next: "/creators" } });
                        return;
                      }
