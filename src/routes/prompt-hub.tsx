...
export const Route = createFileRoute("/prompt-hub")({
  head: () => ({
    ...
    links: [{ rel: "canonical", href: pageUrl("/prompt-hub") }, ...heroPreloadLinks],
  }),
  ...
  return (
    <div className="min-h-svh bg-background">
      ...
      <section className="relative overflow-hidden border-b border-border/60">
        ...
        />
        <div
          className="pointer-events-none absolute -left-32 top-10 h-72 w-72 rounded-full opacity-30 blur-3xl"
          style={{ background: "var(--gradient-gold)" }}
        />
        <div
          className="pointer-events-none absolute -right-32 bottom-0 h-80 w-80 rounded-full opacity-20 blur-3xl"
        ...
        <div className="relative mx-auto max-w-[1400px] px-4 py-16 sm:px-8 sm:py-24">
          <Link
            to="/"
            className="mb-8 inline-flex items-center gap-2 py-2 text-xs text-muted-foreground transition hover:text-foreground"
          >
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-amber-300/30 bg-amber-300/5 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-amber-200">
            ...
            {m.explore_platform_map({ count: String(SECTION_COUNT) })}
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              ...
              {m.explore_stat_live({ count: String(liveCount) })}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full bg-amber-300" />
              {m.explore_stat_launching({ count: String(q1Count) })}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full bg-fuchsia-400" />
              {m.explore_stat_indev({ count: String(q2Count) })}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full bg-white/20" />
              {m.explore_stat_vision({ count: String(q3Count) })}
            </span>
          </div>
        </div>
      </section>
          ...
          <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-border/60 pt-6 sm:flex-row">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              <span className="text-gradient-gold">
                {m.footer_sections({ count: String(SECTION_COUNT) })}
              </span>{" "}
              · {m.footer_powered()}
            </p>
          </div>
