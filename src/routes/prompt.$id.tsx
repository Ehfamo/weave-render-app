...
export const Route = createFileRoute("/prompt/$id")({
  ...
  head: ({ loaderData, params }) => {
    if (!loaderData) {
      return {
        meta: [{ title: "Prompt not found — XeomX" }, { name: "robots", content: "noindex" }],
      };
    }
  ...
  notFoundComponent: () => (
    <div className="grid min-h-screen place-items-center bg-background px-4 text-center">
      <div className="max-w-md">
        ...
        <p className="mt-3 text-sm text-muted-foreground">{m.root_404_desc?.() ?? ""}</p>
    ...
    queryFn: async () => {
      if (!promptId) {
        ...
        return PROMPTS.filter(
          (p: Prompt) => p.id !== prompt.id && p.category === prompt.category,
        ).slice(0, 4);
      }
      const res = await fetchPromptBySlug(params.id);
      if (!res) return [];
  ...
  const onShare = async () => {
    ...
    try {
      if (
        ...
        (navigator as unknown as { share?: (d: { url: string; title?: string }) => Promise<void> })
          .share
      ) {
        await (
          navigator as unknown as { share: (d: { url: string; title?: string }) => Promise<void> }
        ).share({ url, title: prompt.title });
    ...
    <div className="min-h-screen bg-background text-foreground">
                    ...
                    prompt.state === "premium"
                      ...
                      : prompt.state === "soon"
                        ? {
                            ...
                            color: "var(--magenta)",
                            border:
                              "1px solid color-mix(in oklab, var(--magenta) 40%, transparent)",
                          }
                        : {
            ...
            <div>
              ...
              >
                {[
                  ...
                  <div key={label} className="surface-raised rounded-xl p-3">
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {locked
                      ? m.prompt_locked_button()
                      : copied
                        ? m.prompt_copied_button()
      ...
      <section className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.28em] text-magenta/80">
              {m.prompt_related_eyebrow()}
            </p>
            <h2 className="mt-1 font-display text-2xl font-semibold tracking-tight sm:text-3xl">
              {m.prompt_related_title()}
            </h2>
          </div>
