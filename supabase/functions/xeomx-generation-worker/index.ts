...
function researchSources(job: JobInput): ResearchSource[] {
  const metadata = job?.request_metadata;
  if (!isRecord(metadata) || metadata.mode !== "research-v1" || !Array.isArray(metadata.sources))
    return [];
  return metadata.sources
    .filter(
      (source: unknown): source is ResearchSource =>
        isRecord(source) &&
...
function promptForJob(job: JobInput, question: string): GenerateInput {
...
async function processOne() {
  const admin = createAdmin();
  const recovered = await admin.rpc("xeomx_fail_stale_generation_jobs", {
    p_lease_seconds: 600,
    p_limit: 25,
  });
  if (recovered.error) throw recovered.error;
  if (Number(recovered.data ?? 0) > 0) {
    console.warn(
      "xeomx-stale-generation-leases-recovered",
      JSON.stringify({ recovered_count: Number(recovered.data) }),
    );
  }
  ...
  for (let index = 0; index < routes.length; index += 1) {
    ...
    if (!route.configured()) {
      previousFailure = new ProviderFailure(
        "PROVIDER_UNAVAILABLE",
        `${route.id} is not configured`,
      );
    ...
    try {
      ...
      if (completed.error) {
        const persistenceFailure = new ProviderFailure(
          "GENERATION_FAILED",
          "generation persistence failed",
        );
        console.error(
          "xeomx-provider-success-persistence-failed",
          JSON.stringify({
            job_id: job.id,
            provider: route.id,
            model: route.model,
            provider_request_id_present: Boolean(output.providerRequestId),
          }),
        );
        await admin.rpc("xeomx_fail_generation_job", {
          p_job_id: job.id,
          p_error_category: persistenceFailure.code,
          p_safe_error_message: persistenceFailure.message,
          p_provider_request_identifier: output.providerRequestId,
        });
        return {
          processed: true,
          status: "failed",
          reason: "persistence_failed_after_provider_success",
          provider: route.id,
          model: route.model,
        };
      }
...
Deno.serve(async (req) => {
  ...
  EdgeRuntime.waitUntil(
    processOne().catch(() => {
      console.error("xeomx-worker-unhandled-failure", JSON.stringify({ code: "WORKER_FAILED" }));
    }),
  );
  return Response.json(
    { accepted: true },
