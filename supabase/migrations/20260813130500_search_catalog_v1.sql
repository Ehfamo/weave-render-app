...
-- This staging branch intentionally restores only the existing marketplace entities
-- required by live Search: profiles, prompts, collections and collection_items.
-- Table/constraint names match the repository's original marketplace schema.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'prompts_author_id_profiles_fkey'
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'collections_owner_id_profiles_fkey'
