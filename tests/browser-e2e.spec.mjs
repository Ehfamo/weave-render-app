...
test("real browser completes and reloads the Request7 Cloudflare vertical slice", async ({ page }) => {
  const consoleErrors = [];
  ...
  await page.getByPlaceholder("Description").first().fill("Real authenticated browser vertical slice verification");
  await page.getByRole("button", { name: /^Create$/ }).click();
  await expect(page.getByRole("button", { name: new RegExp(projectName) })).toBeVisible({ timeout: 20_000 });
  ...
  await expect(page.getByRole("heading", { name: "XEOMX Project Workspace" })).toBeVisible();
  await expect(page.getByText("XEOMX LIVE VERIFIED", { exact: true })).toBeVisible({ timeout: 20_000 });
  await expect(page.getByText(/Credits:\s*24/)).toBeVisible();

  await page.screenshot({ path: "test-results/xeomx-request7-live.png", fullPage: true });
